...
In our research, we want to find what feature set will improve the machine learning bot's performance the most in Schnapsen. There is already a machine learning bot present, which acts as a base line - with what we will call, the \textit{default feature space}- provided by the framework.  \\
...
\noindent
Our goal is to build multiple bots machine learning bots, both with extended feature spaces and narrower feature spaces, and see which bot will perform best. With \textit{performs best}, we mean which bot will have the most wins against the other bots (rdeep, bully, rand, minimax, kbbot). Our hypothesis is that the bots with an extended feature space will outperform the bots with a narrower feature space. The default machine-learning bot will probably lie somewhere in the middle of the bots in terms of performance.

\section{Machine Learning Bots Feature Space}
For the purpose of our research, we created 5 machine-learning bots accompanying the original machine-learning bot. Below, we will give an overview of the bots and the changes made. For the whole overview, refer to appendix A:

\subsection{ ml\_stripped}
This bot stripped-down version of the original ml-bot. Its feature space only contains the perspective, and the point count of both player 1 and player 2.

\subsection{ml\_minimal}
...

\subsection{ml\_advancedSimple}
This bot has a slightly extended feature space in comparison to the original ml-bot. The added features are player 1 and player 2 points are squared and cubed and added to the feature space. The same goes for the pending points, these are also squared and cubed, and added to the feature space, in order to get more of a difference in the data points.

\subsection{ml\_advanced}
This bot has a more extended feature space. Added are differences between points, differences between pending points, trump card ratio in the hand, high-ranking card ratio (10's or A's) in the hand, number of trump cards in phase 2, number of same suit cards as opponent's played card, and number of points currently in the hand $^2$. 

\subsection{ml\_combined} 
The ml-combined bot combines, the advancedSimple and advanced ml-bot, creating a bot with all feature space extensions. 

\section{Experimental setup}

WORK IN PROGRESSSS

To answer our research question, multiple Schnapsen tournaments were held. Each learning bot played multiple 120 game tournaments against each individual non-learning bot. Each learning bot played a total of 9 tournaments with 3 different models against each static bot.
\subsection{The bots}
The non- learning opponents were Rand, Bully and Rdeep. The learning components we created were:
\begin{description}
\item [ml\_minimal:] asdf
\item [ml\_stripped:]  asdf
\item [ml:] This ml bot is provided by the framework and consists of the  \textit{default feature space} 
\item [ml\_enriched:]  asdf
\item [ml\_advanced:] asdf
\end{description}
...
This procedure was repeated twice to reach the total of 9 tournaments respectively. The machine-learning bots did not compete against eachother.
